...
//  ZOBA
//
//  Created by Angel mas on 7/2/16.
...
class FirebaseLogin : UIViewController ,FBSDKLoginButtonDelegate , UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        //
        print("===============================")
        let op = FireBaseDataBaseOperations()
      //  op.saveUserData(SessionObjects.currentUser)
        op.updateVehicleData()
        print("===============================")
    ...
    @IBAction func LoginTapped(sender: UIButton) {
        ...
        FIRAuth.auth()?.signInWithEmail(emailTextField.text!, password: passwordTextField.text!) { (user, error) in
            ...
            if error != nil {
                print("error in login firebase user")
                print(error)
            }
            else {
                print("done login  new user to firebase")
                print(user)
    ...
    @IBAction func registerTapped(sender: UIButton) {
        ...
        FIRAuth.auth()?.createUserWithEmail(emailTextField.text!, password: passwordTextField.text!) { (user, error) in
            ...
            if error != nil {
                print("error in register firebase user")
                print(error)
            }
            else {
                print("done registering new user to firebase")
                print(user)
    ...
    func loadUserDataFromFireBase(){
        ...
        op.loadUserData(SessionObjects.currentUser.userName!) { (user, code) in
            if code == "success"
            {
                user.save()
            }
